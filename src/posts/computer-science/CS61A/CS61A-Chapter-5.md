...
+ 此外，在设计递归函数的结构时，也使用到了函数抽象——不考虑`fact(n-1)`的具体实现方式，先假定`fact(n-1)`的返回值就是$(n-1)!$。这就将递归函数正确性的验证转化为归纳法（Induction）的形式（可参见[CS70 Chapter 3](/posts/computer-science/cs70/cs70-chapter-4/)）
