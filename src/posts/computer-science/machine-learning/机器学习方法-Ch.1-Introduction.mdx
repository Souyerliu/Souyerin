...
+ **Bias-Variance Trade-off**（偏差-方差权衡）
    + 对于给定的测试样本$x_0$，其测试均方误差可分解为：
        $$
        \begin{aligned}
        \mathbb{E} \left( y_0 - \hat{f}(x_0) \right)^2 &=\mathbb{E} \left( y_0 - \mathbb{E}\hat{f}(x_0) \right)^2 + \mathbb{E} \left(\hat{f}(x_0)-\mathbb{E}\hat{f}(x_0)\right)^2+\mathbb{E}(\epsilon^2)\\
...
### 分类模型
1. 参数模型
    + 以**逻辑斯蒂回归**（Logistic Regression）为例：
    $$
    \begin{aligned}
    P(y=1|x)&=\sigma(wx+b)\\
...
2. 非参数模型：
    ...
    + 理论最优的分类器是**贝叶斯分类器**（Bayes Classifier）：
        ...
        + 最小化：$\mathbb{E}\left[L(g_k, \hat{G}(x))\right]$
...
+ 是一条横坐标$FP$，纵坐标$TP$的曲线，表示不同参数/分类准则下，$TP$和$FP$的关系。
    + 参考图如下（图源[维基百科](https://zh.wikipedia.org/wiki/ROC%E6%9B%B2%E7%BA%BF)）：![ROC](./机器学习方法-Ch.1-Introduction/Roc_curve.png)
...
+ 解决方案：使用K-fold交叉验证，即将数据均分为$K$份，其中$K-1$份用于训练，剩下$1$份用于验证。
    ...
    + 当$K=n$（样本量）时称为留一法交叉验证（LOOCV），其估计效果不一定优于K-fold（$K$一般取5或10）
