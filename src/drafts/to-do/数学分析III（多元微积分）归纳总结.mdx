...
4. 判断下列函数在$(0,0)$处的连续性、任意方向导数以及全微分的存在性，若存在则求解其相应的任意方向导数值以及全微分： {.quiz .fill}
   $$
...
5. **多元函数微分学：** {.quiz}
6. 求函数$u=xyz$在沿点$A(5,1,2)$到点$B(9,4,14)$的方向$\overrightarrow{AB}$上的方向导数. [$\frac{98}{13}$]{.gap} {.quiz .fill}
   ...
   \end{aligned}
   > $$
7. 设$u=f(\frac{x}{y},\frac{y}{z})$，求$\frac{\partial u}{\partial x},\frac{\partial u}{\partial y},\frac{\partial u}{\partial z}$。{.quiz .fill}
   ...
   \end{aligned}
   > $$
8. 已知函数$u=(xyz)e^{x+y+z}$，求$\frac{\partial^{p+q+r}u}{\partial x^p\partial y^q\partial z^r}$.  {.quiz .fill}
   > 注意到
   > $$
...
9. 证明：{.quiz .fill}
   ...
   > $$
      ...
      \end{aligned}
   > $$
10. 已知$z=f(x+y,xy,\frac{x}{y})$，求$z_x,z_{xx},z_{xy}$.{.quiz .fill}
   > $$
...
12. 求由下列方程所确定的隐函数的导数：{.quiz .fill}
   ...
   > $$
13. 已知$u=u(x,y),v=v(x,y)$满足下列方程组：{.quiz .fill}
   $$
   \left\{
      \begin{aligned}
         u=f(ux,v+y)\\
...
14. 求下列函数组所确定的反函数组的偏导数： {.quiz .fill}
   $$
   \left\{\begin{aligned}
      &x=u+v\\
      &y=u^2+v^2\\
   ...
   > $$
   > 法2：<Spoiler title="你知道的太多了">严格来说上面的方法属于“投机取巧”，并未完全使用反函数组的性质</Spoiler>  
...
15. 已知空间中的一个平面 {.quiz .fill}
   ...
   > $$
16. 求函数$f(x,y,z)=4x^2+y^2+5z^2$在平面$2x+3y+5z=12$上的最小值点。{.quiz .fill}
...
17. **含参量积分：** {.quiz}
18. 设$F(x,y)=\int_{\frac{x}{y}}^{xy}(x-yz)f(z)dz$，其中$f(z)$为可微函数，求$F_{xy}(x,y)$。{.quiz .fill}
19. 运用对参量的微分法，求下列积分：{.quiz .fill}
   $$
...
21. 证明下列函数关于$\alpha$在集合$-1<\alpha<1$上内闭一致收敛： {.quiz .fill}
   $$
...
22. **曲线积分：** {.quiz}
23. 若曲线以极坐标$\rho=\rho(\theta)(\theta_1\leq\theta\leq\theta_2)$表示，试给出计算$\int_L f(x,y)ds$的公式，并用此公式计算下列曲线积分：{.quiz .fill}
   $$
   ...
   \end{aligned}
   > $$
24. 计算下述曲线积分： [$-\frac{\pi}{4}a^3$]{.gap} {.quiz .fill}
   $$
   ...
   > $$ 
25. **多重积分：** {.quiz}
26. 设平面区域$D$在$x$轴和$y$轴的投影长度分别为和$l_x$和$l_y$，$D$的面积为$S_D$，$(\alpha,\beta)$为$D$内任一点，证明：     
   （1）{.quiz .fill} 
   $$
...
27. 运用格林公式计算双纽线$(x^2+y^2)^2=a^2(x^2-y^2)$所围的平面面积。{.quiz .fill}
