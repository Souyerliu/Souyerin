...
+ 由前向-后向向量，我们可以得到特征函数的期望：
    $$
    \begin{aligned}
    \mathbb{E}_{P(\mathbf{y}|\mathbf{x})}[f_j]
    ...
    \mathbb{E}_{\tilde{P}(\mathbf{x}, \mathbf{y})}[f_j]&= \sum_{\mathbf{x}, \mathbf{y}} \tilde{P}(\mathbf{x}, \mathbf{y})f_j(\mathbf{y}, \mathbf{x}) \\
...
+ 与HMM类似，线性链CRF的学习算法同样有监督学习算法（极大似然估计法）：
    + 对于观测序列与标记序列均一致的训练数据，其联合概率可表达为
        ...
        $$
        \begin{aligned}
        \frac{\partial L}{\partial w_j} &= \sum_{\mathbf{x},\mathbf{y}} \tilde{P}(\mathbf{x}) P_{\mathbf{w}}(\mathbf{y}|\mathbf{x}) f_j(\mathbf{y},\mathbf{x}) - \mathbb{E}_{\tilde{P}(\mathbf{x}, \mathbf{y})}[f_j], \\
        &=\sum_{\mathbf{x},\mathbf{y}} \tilde{P}(\mathbf{x})\cdot \mathbb{E}_{P(\mathbf{y}|\mathbf{x})}[f_j]- \mathbb{E}_{\tilde{P}(\mathbf{x}, \mathbf{y})}[f_j], \quad j = 1, 2, \cdots, M
        \end{aligned}
        $$
