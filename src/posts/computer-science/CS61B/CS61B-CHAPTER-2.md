...
- 当然，与实例变量相对的，在类中也可以定义静态变量。示例如下：
  ...
  ```
- 与静态方法类似，定义静态变量后，调用变量需要以`类名.变量名`的形式，而不能通过对象名调用变量。另外，静态变量的值只能通过类进行修改，因此**谨慎使用**。
> 注：在Java代码规范中，除了明确声明为常量（使用`public static final`），其他情况下定义变量（无论静态还是非静态）一般都不使用`public`，而使用`private`替代。
