...
+ 在python中，常使用[nltk](https://www.nltk.org/)的`tokenize`库对文本进行词元化。（可参见[官方文档](https://www.nltk.org/api/nltk.tokenize.html#module-nltk.tokenize)）
    + 示例：
        ```python
...
+ BPE的训练算法主要通过迭代合并高频相邻词元，逐步生成越来越长的词元。具体而言，算法从一个仅包含所有单个字符的初始词表开始，不断扫描训练语料，找出出现频率最高的两个相邻字符（或词元）。
    + 其中，合并操作（生成新词元）次数$k$是BPE算法的一个重要的超参数。
...
### Unicode
+ Unicode的编码方式类似ASCII编码，被称为**码位（code point）**。
...
### 大模型BPE
+ 大预言模型语言模型通常会在BPE之前先进行预分词，使用正则表达式分割输入，例如在空格和标点处分割文本、剥离附着词（如`'s`，`n't`）、将长数字按三位分组等。
