...
tags:
  ...
  - 查找
  - 排序
cover: ./ECNU 2024-2025学年数据结构期末试题/cover.jpg
---
1. **一、多项选择题（5分/题x10题=50分）**  {.quiz}
{.options} 
...
1. 在二叉树中，下列情况可能导致树的高度增加的有： {.quiz .multi}
    - 插入一个叶子节点 {.correct}
    - 删除一个度为$1$的节点
    - 插入一个非叶子节点 {.correct}
    - 删除一个叶子节点
...
1. 以下关于排序算法的说法中，正确的有：  {.quiz .multi}
...
1. 以下关于深度优先搜索（DFS）的描述正确的有： {.quiz .multi}
    - 它使用栈实现 {.correct} 
...
1.   **二、编程题**（1.请用C或C++语言；2.不同小题的源代码请标识清楚所在题号；3.若无法提供源代码，请用自然语言描述思路；4.请将能运行出的结果截**全屏**展示） {.quiz .true}
2.    **第1题(25分)：** 在一个学生成绩管理系统中，需要对学生的课程成绩进行管理。每个学生有多门课程的成绩，课程信息包括课程名称、课程编号、学分，学生信息包括学号、姓名、性别。 {.quiz .true}
3.   **小题1（5分）**：定义课程结构体，包含课程名称、课程编号、学分。 {.quiz .fill}
     > ```cpp
     > typedef struct{
     ...
     > }course;
     > ```
4.   **小题2（5分）**：定义学生结构体，包含学号、姓名、性别以及一个课程结构体数组，用于存储该学生的多门课程信息。 {.quiz .fill}
     > ```cpp
     > typedef struct{
     >     int id;
     ...
     > }student;
     > ```
5.   **小题3（5分）**：编写一个函数，输入学生数量，然后依次输入每个学生的信息（包括学号、姓名、性别以及每门课程的课程名称、课程编号、学分），并存储到学生结构体数组中。  {.quiz .fill}
     > ```cpp
     > student s[105];
     > int n,cnt[105];
     > void read_in(){
     ...
     > }
     > ```
6.   **小题4（5分）**：编写一个函数，在已存储学生信息的结构体数组中，根据学号查找（请指出使用的查找算法名称）某个学生，并输出该学生的所有课程信息。 {.quiz .fill}
     > ```cpp
     ...
     > }
     > ```
7.   **小题5（5分）**：编写一个函数，计算每个学生的总学分，并在结构体数组中增加一个成员变量来存储总学分，最后输出所有学生的学号、姓名以及总学分。 {.quiz .fill}
     > ```cpp
...
8.  最后附上主函数与测试样例：{.quiz .fill}
     ...
     > */
     > ```
9.   **第2题(25分)：** 在一个物流运输系统中，用图来表示城市之间的运输路线。图中的节点表示城市，边表示运输路线，边的权值表示运输费用。 {.quiz .true}
10. **小题1（4分）**：定义一个城市结构体，包含城市名称和城市编号。 {.quiz .fill}
     > ```cpp
     > typedef struct{
     >     char name[105];
     >     int id;
     > }city;
     > ```
11. **小题2（4分）**：使用邻接矩阵来存储该物流运输图，定义图的结构体，包含城市结构体数组和邻接矩阵。 {.quiz .fill}
     > ```cpp
     > typedef struct{
     >     city c[1005];
     >     int g[1005][1005];
     > }graph;
     > ```
12. **小题3（4分）**：编写一个函数，输入城市数量，然后依次输入每个城市的名称和编号，初始化图的城市结构体数组。 {.quiz .fill}
     > ```cpp
     > int n;
     > city ci[1005];
     > void read_city(){
     ...
     > }
     > ```
13. **小题4（4分）**：编写一个函数，输入运输路线数量，然后依次输入每条路线的起点城市编号、终点城市编号以及运输费用，填充邻接矩阵。 {.quiz .fill}
     > ```cpp
     > graph transfer;
     > void fill_g(){
     ...
     > }
     > ```
14. **小题5（4分）**：编写一个函数，输出从指定城市出发，能够直接到达的所有城市及其运输费用。 {.quiz .fill}
     > ```cpp
     > int city_id;
     ...
     > }
     > ```
15. **小题6（5分）**:编写一个函数，使用Floyd算法计算所有城市之间的最短运输费用路径，并输出任意两个城市之间的最短路径和费用。 {.quiz .fill}
     > ```cpp
